...
function procesoComentarios(){
	...
	let search = document.getElementById('buscar');
		buscar.addEventListener('submit', (event) =>{
			...
			event.preventDefault();
				...
				let url = 'http://localhost:8080/blog-api/comentarios-por-autor?autor='+document.getElementById('autor').value;
				let settings = {
					method : "GET"
				}
				fetch(url, settings)
	...
	let actualizarC = document.getElementById('submitA');
		actualizarC.addEventListener('click', (event) =>{
			let url = "http://localhost:8080/blog-api/actualizar-comentario/"+document.getElementById('actualizarId').value;
	...
	let comBorrar = document.getElementById('submitR');
		comBorrar.addEventListener('click', (event) =>{
				...
				let url = "http://localhost:8080/blog-api/remover-comentario/"+document.getElementById('removerId').value;
				let settings = {
					method : "DELETE",
				}
				fetch(url, settings)
